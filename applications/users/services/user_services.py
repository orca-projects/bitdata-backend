...
class UserServices:
    ...
    def save_last_collected(binance_id, last_collected):
        active_user_key_info = UserKeyInfoRepository.find_active_by_binance_id(
            binance_id
        )
        UserKeyInfoRepository.update_user_key_info(
    ...
    def save_positions_data(positions_data):
        PositionOrdersRepository.save_positions_data(positions_data)
    ...
    def process_positions(binance_id, start_position_id, all_orders_data):
        ...
        for _, row in df_orders.iterrows():
            ...
            formatted_order_id = (
                f"{symbol}_{row['orderId']}"  # `{SYMBOL}_orderId` 형식으로 변환
            )
    ...
    def process_transactions(binance_id):
        ...
        for position_id, pos_orders in position_groups.items():
            ...
            order_numbers = [int(pos.order_id.split("_")[1]) for pos in pos_orders]
            ...
            if min_order and max_order:
                ...
                if orders:
                    ...
                    if first_order and last_order:
                        ...
                        total_buy = (
                            sum(
                                float(order.executed_qty) * float(order.avg_price)
                                for order in orders
                                if order.side == target_buy_side
                            )
                            if target_buy_side
                            else 0.0
                        )
                        ...
                        total_sell = (
                            sum(
                                float(order.executed_qty) * float(order.avg_price)
                                for order in orders
                                if order.side == target_sell_side
                            )
                            if target_sell_side
                            else 0.0
                        )
                        ...
                        trades = (
                            TradesRepository.get_trades_by_binance_id_and_order_ids(
                                binance_id, buy_order_ids
                            )
                        )
